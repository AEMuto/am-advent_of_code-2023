...
const day1_part1 = (data) => {
    return data.reduce((acc, next) => {
        //console.log(next);
        const numArr = next.match(/\d/g);
        //console.log(numArr);
        const digits = parseInt(`${numArr[0]}${numArr[numArr.length - 1]}`, 10);
        //console.log(digits);
        acc += digits;
        return acc;
    }, 0);
...
const convert = (str) => dict[str] ?? str;

const day1_part2 = (data) => {
    return data.reduce((acc, next) => {
        // console.log(next);
        const arr = [];
