...
const result_day_3 = INPUT.reduce(
    (acc, string, y) => {
        string.split("").forEach((char, x) => {
            // regex to find special characters excluding the dot
            const re = /[!"#$%&'()*+,\-/:;<=>?@[\\\]^_`{|}~]/g;
            if (re.test(char)) {
                const surrounding_chars = [
                    [   // Above line
                        INPUT[y - 1][x - 3] ?? null,
                        INPUT[y - 1][x - 2] ?? null,
                        INPUT[y - 1][x - 1] ?? null,
                    ...
                    [   // Current line
                        ...
                        INPUT[y][x - 2] ?? null,
                        INPUT[y][x - 1] ?? null,
                        INPUT[y][x], // Position of the special char
                        INPUT[y][x + 1] ?? null,
                        INPUT[y][x + 2] ?? null,
                        INPUT[y][x + 3] ?? null,
                    ].join(""),
                ...
                console.log(
                    "Special char found, surrounding characters are: ",
                    surrounding_chars,
                );

                const num_regex = new RegExp("[0-9]+", "g");
                const valid_numbers = [];

                surrounding_chars.forEach((line) => {
                    const matches = Array.from(line.matchAll(num_regex));
                    if (matches.length > 0) {
                        matches.forEach((num_match) => {
                            const length = num_match[0].length;
                            if (
                                length === 1 &&
                                num_match.index >= 2 &&
                                num_match.index <= 4
                            ) { // 1 digit numbers are valid their start index is between 2 and 4
                                console.log(
                                    `Valid Part number found: ${num_match[0]}, adding to result: ${acc.part_one}`,
                                );
                                valid_numbers.push(parseInt(num_match[0], 10));
                            }
                            if (
                                length === 2 &&
                                num_match.index >= 1 &&
                                num_match.index <= 4
                            ) { // 2 digits numbers are valid their start index is between 1 and 4
                                console.log(
                                    `Valid Part number found: ${num_match[0]}, adding to result: ${acc.part_one}`,
                                );
                                valid_numbers.push(parseInt(num_match[0], 10));
                            }
                            if (length === 3) { // 3 digits numbers are always valid
                                console.log(
                                    `Valid Part number found: ${num_match[0]}, adding to result: ${acc.part_one}`,
                                );
                                valid_numbers.push(parseInt(num_match[0], 10));
                ...
                if (char === "*" && valid_numbers.length === 2) {
                    const ratio = valid_numbers[0] * valid_numbers[1];
                    acc.part_two += ratio;
                }
                // Resolving part one
                acc.part_one += valid_numbers.reduce((a, b) => a + b, 0);
            }
        });
...
console.log("Part one", result_day_3.part_one);
console.log("Part two", result_day_3.part_two);
