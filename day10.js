...
const validNorth = ["|", "7", "F"]
const validSouth = ["|", "L", "J"]
const validEast = ["-", "7", "J"]
const validWest = ["-", "L", "F"]

const START = {
  y: MAZE.findIndex(row => row.includes("S")),
  x: MAZE.find(row => row.includes("S")).indexOf("S")
}
...
const sameCoordinates = (pos1, pos2) => pos1.x === pos2.x && pos1.y === pos2.y

const getValidMoves = (current_xy, previous_xy) => {
  const current_tile = MAZE[current_xy.y][current_xy.x]
  const tiles_around = lookAround(current_xy)

  switch (current_tile) {
    case "S":
      return Object.values(tiles_around).filter(direction => {
        if (direction.position === "north") return validNorth.includes(direction.tile)
        if (direction.position === "south") return validSouth.includes(direction.tile)
        if (direction.position === "east") return validEast.includes(direction.tile)
        if (direction.position === "west") return validWest.includes(direction.tile)
      })
    case "|": // South or North
      return [tiles_around.south, tiles_around.north]
    ...
    case "J": // North or West
      return [tiles_around.north, tiles_around.west]
        .filter(move => !sameCoordinates(move.coordinates, previous_xy))
    case "7": // South or West
      return [tiles_around.south, tiles_around.west]
        .filter(move => !sameCoordinates(move.coordinates, previous_xy))
    case "F": // South or East
      return [tiles_around.south, tiles_around.east]
        .filter(move => !sameCoordinates(move.coordinates, previous_xy))
    default:
...
const traverseMaze = () => {
  ...
  do {
    ...
    let validMoves = getValidMoves(currentPos, previousPos)[0]
    //console.log("Valid Moves: ", validMoves)
    let nextPos = validMoves.coordinates
    //console.log("Next Position: ", nextPos)
    path.push(nextPos)
    previousPos = currentPos
    currentPos = nextPos
    if (currentPos.x === START.x && currentPos.y === START.y) {
      console.log("BREAK, we're back at the start!")
      break
    }
  } while (true)
  return path
