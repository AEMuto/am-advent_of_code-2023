...
const lookAround = (pos) => {
    const { x, y } = pos
    return {
        north: [maze[y - 1][x], { x, y: y - 1 }],
        south: [maze[y + 1][x], { x, y: y + 1 }],
        east: [maze[y][x + 1], { x: x + 1, y }],
        west: [maze[y][x - 1], { x: x - 1, y }],
    }
}

// const validMoves = (pos) => {
//     const directions = lookAround(pos)
//     const valid = []

const traverseMaze = () => {
    let currentPos = start
    let validMoves = lookAround(currentPos)
}



console.log(lookAround(start))